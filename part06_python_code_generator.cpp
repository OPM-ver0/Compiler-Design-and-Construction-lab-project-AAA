...
private:
    ...
    string pyName(const string& sourceName) const {
        return "sbb_" + sourceName;
    }

    string normalizeNumber(const string& value) const {
        size_t dot = value.find('.');
        if (dot == string::npos) {
            size_t first = value.find_first_not_of('0');
            return first == string::npos ? "0" : value.substr(first);
        }

        string whole = value.substr(0, dot);
        string fraction = value.substr(dot);
        size_t first = whole.find_first_not_of('0');
        if (first == string::npos) whole = "0";
        else whole = whole.substr(first);
...
public:
    string generate(const Program &program) {
        ...
        types.clear();
        indent = 0;
        out << "# Generated by Sadhu Bangla Compiler\n";
