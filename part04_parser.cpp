...
private:
    ...
    DataType parseType() {
        if (match(TokenType::KW_PURNO)) return DataType::INT;
        if (match(TokenType::KW_DOSHOMIK)) return DataType::DOUBLE;
        if (match(TokenType::KW_OKKHORMALA)) return DataType::STRING;

        failAt(peek(), "Expected a data type (purno, doshomik, or okkhormala)");
    }
    ...
    StmtPtr printStatement() {
        int stmtLine = peek().line;
        advance(); // mudron
        consume(TokenType::LPAREN, "Expected '(' after mudron");
        ExprPtr expr = expression();
        consume(TokenType::RPAREN, "Expected ')' after mudron argument");
        consume(TokenType::SEMICOLON, "Expected ';' after mudron statement");
    ...
    StmtPtr ifStatement() {
        ...
        if (match(TokenType::KW_ONNOTHAY)) {
    ...
    StmtPtr whileStatement() {
        int stmtLine = peek().line;
        advance(); // jotokkhon
        consume(TokenType::LPAREN, "Expected '(' after jotokkhon");
        ExprPtr condition = expression();
        consume(TokenType::RPAREN, "Expected ')' after condition");
        auto body = block();
    ...
    StmtPtr continueStatement() {
        int stmtLine = peek().line;
        advance(); // ogrosor
        consume(TokenType::SEMICOLON, "Expected ';' after ogrosor");
    ...
    StmtPtr breakStatement() {
        int stmtLine = peek().line;
        advance(); // sthogito
        consume(TokenType::SEMICOLON, "Expected ';' after sthogito");
        return make_unique<BreakStmt>(stmtLine);
    }

    StmtPtr statement() {
        if (checkAny({TokenType::KW_PURNO, TokenType::KW_DOSHOMIK, TokenType::KW_OKKHORMALA}))
            return declaration();
        if (check(TokenType::KW_MUDRON)) return printStatement();
        if (check(TokenType::KW_JODI)) return ifStatement();
        if (check(TokenType::KW_JOTOKKHON)) return whileStatement();
        if (check(TokenType::KW_OGROSOR)) return continueStatement();
        if (check(TokenType::KW_STHOGITO)) return breakStatement();
