...
static string dataTypeName(DataType t) {
    switch (t) {
        case DataType::INT: return "purno";
        case DataType::DOUBLE: return "doshomik";
        case DataType::STRING: return "okkhormala";
...
struct IfStmt : Stmt {
    ...
    // `onnothay jodi (...) { ... }` is represented as another IfStmt.
    // A final plain `onnothay { ... }` is stored in elseBranch.
