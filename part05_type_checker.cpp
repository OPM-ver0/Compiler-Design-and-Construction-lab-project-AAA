...
private:
    ...
    void checkStatement(Stmt *stmt) {
        ...
        if (auto *s = dynamic_cast<WhileStmt*>(stmt)) {
            DataType t = checkExpr(s->condition.get(), s->line);
            if (t != DataType::BOOL && t != DataType::ERROR) {
                error(s->line, "jotokkhon condition must be a boolean expression");
            }
        ...
        if (dynamic_cast<ContinueStmt*>(stmt)) {
            if (loopDepth == 0) {
                error(stmt->line, "'ogrosor' can only be used inside a jotokkhon loop");
            }
            return;
        }
        ...
        if (dynamic_cast<BreakStmt*>(stmt)) {
            if (loopDepth == 0) {
                error(stmt->line, "'sthogito' can only be used inside a jotokkhon loop");
            }
            return;
        }
    }
